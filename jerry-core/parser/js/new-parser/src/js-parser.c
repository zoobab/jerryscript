                       ...
                       size_t length) /**< length of byte code */
{
  ...
  byte_code_start_p += literal_end * sizeof (literal_value_t);
...
parser_post_processing (parser_context_t *context_p) /**< context */
{
  ...
  total_size += length + context_p->literal_count * sizeof (literal_value_t);
                       ...
                       uint32_t status_flags) /**< extra status flags */
{
  ...
  if (context_p->status_flags & PARSER_NAMED_FUNCTION_EXP)
  {
    ...
    literal_value_t *literal_pool_p = (literal_value_t *) literal_pool_start_p;
    literal_pool_p[const_literal_end].compiled_code_p = compiled_code_p;
  }
