 ...
 */
static void
parser_emit_unary_lvalue_opcode (parser_context *context_p, /**< context */
    ...
    switch (context_p->token.type)
    {
      ...
      case LEXER_LEFT_SQUARE:
      {
        ...
        if (context_p->last_cbc_opcode == CBC_PUSH_LITERAL
            && context_p->last_cbc.value == LEXER_STRING_LITERAL)
        {
      ...
      case LEXER_LEFT_PAREN:
      {
        ...
        cbc_argument argument = context_p->last_cbc;
        uint8_t opcode;
...
parser_append_binary_token (parser_context *context_p) /**< context */
{
  ...
  {
    cbc_opcode opcode = LEXER_BINARY_LVALUE_OP_TOKEN_TO_OPCODE (context_p->token.type);
  ...
  {
    cbc_opcode opcode = LEXER_BINARY_OP_TOKEN_TO_OPCODE (context_p->token.type);
    ...
    if (!LEXER_IS_BINARY_LVALUE_TOKEN (token))
    {
      ...
      else
      {
            ...
            || context_p->last_cbc_opcode == CBC_PUSH_LITERAL)
        {
          ...
          context_p->last_cbc.value = context_p->last_cbc.literal_index;
