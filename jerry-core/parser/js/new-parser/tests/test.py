...
class TestResult:
    ...
    def addPass(self, testcase):
        if self.verbose:
            print("%s[PASS] %s\033[0m" % ("\033[1;32m", testcase))
        self.num_pass += 1
        self.addExec()

    def addFail(self, testcase):
        print("%s[FAIL] %s\033[0m" % ("\033[1;31m", testcase))
        self.num_fail += 1
        self.addExec()

    def addError(self, testcase):
        print("%s[ERR ] %s\033[0m" % ("\033[1;35m", testcase))
        self.num_err += 1
        self.addExec()
    ...
    def addSkip(self, testcase):
        print("%s[SKIP] %s\033[0m" % ("\033[34m", testcase))
        self.num_skip += 1
        self.addTest()
    ...
    def report(self):
        print("============================================================")
        print("%sAll tests . . . . . . . . . . . : %5d\033[0m"             % ("\033[1m", self.num_all))
        if self.verbose or self.num_skip > 0:
            print("%s  +-- skipped tests . . . . . . :     + %5d\033[0m"       % ("\033[34m", self.num_skip))
        print("%s  +-- executed tests  . . . . . :     + %5d\033[0m"       % ("", self.num_exec))
        print("%s      +-- passed tests  . . . . :           + %5d\033[0m" % ("\033[1;32m", self.num_pass))
        if self.verbose or self.num_fail > 0:
            print("%s      +-- failed tests  . . . . :           + %5d\033[0m" % ("\033[1;31m", self.num_fail))
        if self.verbose or self.num_err > 0:
            print("%s      +-- errors  . . . . . . . :           + %5d\033[0m" % ("\033[1;35m", self.num_err))
        print("============================================================")
