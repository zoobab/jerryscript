...
static vm_instr_t
generate_opcode (scopes_tree tree, vm_instr_counter_t instr_pos, lit_id_hash_table *lit_ids)
{
...
static idx_t
count_new_literals_in_opcode (scopes_tree tree, vm_instr_counter_t instr_pos)
{
  start_new_block_if_necessary ();
...
scopes_tree_count_literals_in_blocks (scopes_tree tree)
{
  ...
  for (instr_pos = 0; instr_pos < tree->instrs_num; instr_pos++)
  {
    ...
    result += count_new_literals_in_opcode (tree, instr_pos);
  }
...
merge_subscopes (scopes_tree tree, vm_instr_t *data, lit_id_hash_table *lit_ids)
{
  ...
  for (instr_pos = 0; instr_pos < tree->instrs_num; instr_pos++)
  {
    ...
    data[global_oc] = generate_opcode (tree, instr_pos, lit_ids);
    global_oc++;
  }
  for (uint8_t child_id = 0; child_id < tree->t.children_num; child_id++)
  {
