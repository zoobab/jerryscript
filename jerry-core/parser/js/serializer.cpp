...
static void
serializer_print_opcodes (const vm_instr_t *instrs_p,
                          size_t instrs_count);
 ...
 */
vm_instr_t
serializer_get_opcode (const vm_instr_t *instrs_p, /**< pointer to byte-code array (or NULL,
...
serializer_merge_scopes_into_bytecode (void)
{
  ...
  const size_t opcodes_array_size = JERRY_ALIGNUP (sizeof (insts_data_header_t) + instrs_count * sizeof (vm_instr_t),
                                                   MEM_ALIGNMENT);
  ...
  const vm_instr_t *instrs_p = scopes_tree_raw_data (current_scope, buffer_p, opcodes_array_size, lit_id_hash);
  ...
  {
    lit_dump_literals ();
    serializer_print_opcodes (instrs_p, bytecode_data.instrs_count);
  }
...
vm_instr_counter_t
serializer_get_current_instruction_counter (void)
{
