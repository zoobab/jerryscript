      ...
      switch (opm.op.data.meta.type)
      {
        ...
        case OPCODE_META_TYPE_VARG_PROP_SETTER:
        {
          ...
          if (seen_vargs == vargs_num)
          {
            ...
            {
              start--;
              switch (serializer_get_opcode (instrs_p, start).op_idx)
              {
                case VM_OP_CALL_N:
                case VM_OP_NATIVE_CALL:
                case VM_OP_CONSTRUCT_N:
            ...
            vm_instr_t start_op = serializer_get_opcode (instrs_p, start);
            ...
            {
              vm_instr_t meta_op = serializer_get_opcode (instrs_p, counter);
