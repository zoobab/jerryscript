                       ...
                       const lit_utf8_size_t buf_size) /**< size in bytes of the buffer which holds the string */
{
  ...
  record_t *rec_p = rcs_alloc_record (rec_set_p, RCS_RECORD_TYPE_CHARSET, aligned_record_size);

  rcs_record_set_alignment_bytes_count (rec_p, (uint8_t) alignment);
  rcs_record_set_charset (rec_set_p, rec_p, str, buf_size);
  rcs_record_set_hash (rec_p, lit_utf8_string_calc_hash (str, rcs_record_get_length (rec_p)));

  return rec_p;
} /* create_charset_record */
 ...
 */
record_t *
create_magic_record (record_set_t *rec_set_p, /**< recordset */
                     lit_magic_string_id_t id) /**< id of magic string */
{
  const size_t record_size = RCS_DYN_STORAGE_LENGTH_UNIT;

  record_t *rec_p = rcs_alloc_record (rec_set_p, RCS_RECORD_TYPE_MAGIC_STR, record_size);
 ...
 */
record_t *
create_magic_record_ex (record_set_t *rec_set_p, /**< recordset */
                        lit_magic_string_ex_id_t id) /**< id of magic string */
{
  record_t *rec_p = rcs_alloc_record (rec_set_p, RCS_RECORD_TYPE_MAGIC_STR_EX, RCS_MAGIC_STR_HEADER_SIZE);
  rcs_record_set_magic_str_ex_id (rec_p, id);

  return rec_p;
} /* create_magic_record_ex */
 ...
 */
record_t *
create_number_record (record_set_t *rec_set_p, /** recordset */
                      ecma_number_t num) /**< numeric value */
{
  const size_t record_size = RCS_NUMBER_HEADER_SIZE + sizeof (ecma_number_t);
  record_t *rec_p = rcs_alloc_record (rec_set_p, RCS_RECORD_TYPE_NUMBER, record_size);
 ...
 */
uint32_t
lit_count_literals (record_set_t *rec_set_p) /**< recordset */
{
  uint32_t num = 0;
  record_t *rec_p;
 ...
 */
void
lit_dump_literals (record_set_t *rec_set_p) /**< recordset */
{
  record_t *rec_p;
