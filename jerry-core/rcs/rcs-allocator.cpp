 ...
 */
static void
rcs_assert_state_is_correct (record_set_t *rec_set_p __attr_unused___) /**< recordset */
{
#ifndef JERRY_DISABLE_HEAVY_DEBUG
  size_t node_size_sum = 0;
  size_t record_size_sum = 0;

  record_t *last_record_p = NULL;
  record_t *rec_p;
  record_t *next_rec_p;
 ...
 */
static void
rcs_init_free_record (record_set_t *rec_set_p, /**< recordset */
                      record_t *rec_p, /**< record to init as free record */
                      record_t *prev_rec_p, /**< previous record (or NULL) */
 ...
 */
void
rcs_check_record_alignment (record_t *rec_p) /**< record */
{
 ...
 */
uint8_t *
rcs_get_node_data_space (record_set_t *rec_set_p, /**< recordset */
                         rcs_chunked_list_t::node_t *node_p) /**< the node */
{
  ...
  JERRY_ASSERT (unaligned_data_space_start + rcs_chunked_list_t::get_node_data_space_size ()
                == aligned_data_space_start + record_set_t::get_node_data_space_size ());
 ...
 */
static void
rcs_alloc_record_in_place (record_set_t *rec_set_p, /**< recordset */
                           record_t *place_p, /**< where to initialize the record */
                           record_t *next_record_p, /**< next allocated record */
                                                  ...
                                                  and the next allocated record */
{
  ...
  if (next_record_p != NULL)
  {
    ...
    else
    {
      ...
      {
        free_rec_p = (record_t *) ((uint8_t *) next_record_p - free_size);
      }
      else
      {
        ...
        free_rec_p = (record_t *) (node_data_space_p + node_data_space_size - \
  ...
  else if (free_size != 0)
  {
    ...
    record_t *free_rec_p = (record_t *) (node_data_space_p + node_data_space_size - free_size);
    rcs_init_free_record (rec_set_p, free_rec_p, place_p, free_size);
  }
 ...
 */
record_t *
rcs_record_get_first (record_set_t *rec_set_p) /**< recordset */
{
  ...
  return (record_t *) rcs_get_node_data_space (rec_set_p, first_node_p);
 ...
 */
record_t *
rcs_record_get_next (record_set_t *rec_set_p, /**< recordset */
                     record_t *rec_p) /**< record */
{
  ...
  {
    return (record_t *) (record_start_p + record_size);
  }
  ...
  return (record_t *) (rcs_get_node_data_space (rec_set_p, node_p) + record_size_left);
} /* rcs_record_get_next */
 ...
 */
static record_t *
rcs_alloc_space_for_record (record_set_t *rec_set_p, /**< recordset */
                            record_t **out_prev_rec_p, /**< out: pointer to record, previous to the allocated,
                            ...
                            size_t bytes) /**< size */
{
  ...
  *out_prev_rec_p = NULL;
       ...
       *out_prev_rec_p = rec_p, rec_p = rcs_record_get_next (rec_set_p, rec_p))
  {
    ...
    {
      record_t *next_rec_p = rcs_record_get_next (rec_set_p, rec_p);
  ...
  record_t *new_rec_p = (record_t *) rcs_get_node_data_space (rec_set_p, node_p);
 ...
 */
record_t *
rcs_alloc_record (record_set_t *rec_set_p, /**< recordset */
                  record_type_t type, /**< type for the new record */
                  size_t size) /**< allocation size */
{
  JERRY_ASSERT (type >= RCS_RECORD_TYPE_FIRST && type <= RCS_RECORD_TYPE_LAST);

  record_t *prev_rec_p;
  record_t *rec_p = (record_t *) rcs_alloc_space_for_record (rec_set_p, &prev_rec_p, size);
 ...
 */
void
rcs_free_record (record_set_t *rec_set_p, /**< recordset */
                 record_t *record_p) /**< record to free */
{
  JERRY_ASSERT (record_p != NULL);

  rcs_assert_state_is_correct (rec_set_p);

  record_t *prev_rec_p = rcs_record_get_prev (rec_set_p, record_p);
  ...
  record_t *rec_from_p = record_p;
  record_t *rec_to_p = rcs_record_get_next (rec_set_p, record_p);
  ...
  {
    for (rcs_chunked_list_t::node_t *iter_node_p = rec_set_p->get_next (node_from_p), *iter_next_node_p;
