 ...
 */
static uint32_t
rcs_snapshot_dump (record_t *rec_p, /**< record to dump */
                   uint8_t *buffer_p, /**< buffer to dump to */
                   size_t buffer_size, /**< buffer size */
                   size_t *in_out_buffer_offset_p) /**< in-out: buffer write offset */
{
  record_type_t record_type = rcs_record_get_type (rec_p);

  if (RCS_RECORD_TYPE_IS_NUMBER (record_type))
  {
    double num_value = rcs_record_get_number (&rcs_lit_storage, rec_p);
  ...
  {
    lit_utf8_size_t length = rcs_record_get_length (rec_p);
    if (!jrt_write_to_buffer_by_offset (buffer_p,
                                        buffer_size,
                                        in_out_buffer_offset_p,
                                        &length,
    ...
    rcs_iterator_ctx_t it_ctx = rcs_iterator_create (&rcs_lit_storage, rec_p);
  ...
  *out_lit_table_size_p = 0;
  ...
  if (literals_num != 0)
  {
         ...
         rec_p = rcs_record_get_next (&rcs_lit_storage, rec_p))
    {
      ...
      uint32_t bytes = rcs_snapshot_dump (rec_p, buffer_p, buffer_size, in_out_buffer_offset_p);
      ...
      cpointer_t lit_cp = rcs_cpointer_compress (rec_p);
 ...
 */
bool
rcs_load_literals_from_snapshot (const uint8_t *lit_table_p, /**< buffer with literal table in snapshot */
  ...
  *out_map_num_p = 0;
  ...
  for (uint32_t lit_index = 0; lit_index < literals_num; lit_index++)
  {
    ...
    if (RCS_RECORD_TYPE_IS_CHARSET (type))
    {
      ...
      lit = (record_t *) lit_find_or_create_literal_from_utf8_string (lit_table_p + lit_table_read, length);
    ...
    else if (RCS_RECORD_TYPE_IS_MAGIC_STR (type))
    {
       ...
       */
      lit = (record_t *) lit_find_or_create_literal_from_utf8_string (magic_str_p, magic_str_sz);
    }
    else if (RCS_RECORD_TYPE_IS_MAGIC_STR_EX (type))
    {
       ...
       *      Consider searching literal storage by magic string identifier instead of by its value
       */
      lit = (record_t *) lit_find_or_create_literal_from_utf8_string (magic_str_ex_p, magic_str_ex_sz);
    }
    else if (RCS_RECORD_TYPE_IS_NUMBER (type))
    {
      ...
      lit = (record_t *) lit_find_or_create_literal_from_num ((ecma_number_t) num);
    }
    else
    {
      is_ok = false;
